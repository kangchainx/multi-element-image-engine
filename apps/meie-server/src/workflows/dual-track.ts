...
export function buildDualTrackWorkflow(opts: {
  ...
  const ids = BASE_WORKFLOW_NODE_IDS;
  ...
  if (workflow[ids.emptyLatent]?.inputs) {
    ...
    if (w) workflow[ids.emptyLatent].inputs.width = Math.trunc(w);
    if (h) workflow[ids.emptyLatent].inputs.height = Math.trunc(h);
  }
  ...
  // ControlNet window.
  if (workflow[ids.controlNetApply]?.inputs) {
    const strength = maybeNumber(opts.params.controlnet_strength);
    const start = maybeNumber(opts.params.controlnet_start);
    const end = maybeNumber(opts.params.controlnet_end);
    if (strength !== undefined) workflow[ids.controlNetApply].inputs.strength = strength;
    if (start !== undefined) workflow[ids.controlNetApply].inputs.start_percent = start;
    if (end !== undefined) workflow[ids.controlNetApply].inputs.end_percent = end;
  }
